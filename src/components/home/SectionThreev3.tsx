 ...
 * stage taller than the screen. Both restore the original document flow, and
 * both zero the travel distance — un-pinned there is not enough scroll left to
 * carry a 60vw slide at any sane speed, so the clips resolve where they sit.
 *
...
export default function SectionThreeV3() {
  ...
  return (
    ...
    // container, so it can hold both clips while they are parked 60vw off-stage
    // without stealing the scrollport that `position: sticky` below resolves
    // against. `hidden` here would break the pin outright.
    //
    // The travel distance is *only ever* declared by the media queries in
    // NO_PIN_SECTION, which zero it. The default lives in the `var()` fallback
    // instead of being set here: an inline style outranks every class rule, so
    // publishing 60vw on this element made the un-pinned override unreachable
    // and left a 60vw slide running through a 111px window on short viewports.
    <section
      ref={sectionRef}
